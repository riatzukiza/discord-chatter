...
print("start building model", settings.model_path)
...
while True:
    messages=get_messages_for_in_order_training(
        random.randint(20,100),
        random.randint(10,100),
        os.path.join(settings.model_path, "config.json")
    )
    ...
    else: print("no messages to train on")
